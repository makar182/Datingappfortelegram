...
import { MapPin, Heart, ChevronLeft, ChevronRight, MessageCircle, Users } from 'lucide-react';
    ...
    return (
      <div className="max-w-lg mx-auto p-6 flex flex-col h-[calc(100vh-5rem)]">
        ...
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl bg-gradient-to-r from-emerald-600 to-teal-600 bg-clip-text text-transparent">Приглашения</h1>
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-3">
          {likesSubTab === 'received' && whoLikedMe.length === 0 ? (
            <div className="bg-white rounded-2xl p-8 text-center shadow-sm">
              <Users className="w-12 h-12 text-gray-300 mx-auto mb-3" />
              <p className="text-gray-500">Пока нет пригашений</p>
            </div>
          ) : likesSubTab === 'mutual' && mutualMatches.length === 0 ? (
            <div className="bg-white rounded-2xl p-8 text-center shadow-sm">
              <p className="text-gray-500">Пока нет взаимных симпатий</p>
              <p className="text-sm text-gray-400 mt-2">Примите приглашение кого-то из тех, кто вас пригласил</p>
            </div>
          ) : (
            (likesSubTab === 'received' ? whoLikedMe : mutualMatches).map((match) => {
              const isLikedBack = myLikes.includes(match.id);
              const isMutual = mutualMatches.find(m => m.id === match.id);
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex gap-2">
                        {likesSubTab === 'received' ? (
                          // В разделе "Лайкнули меня"
                          !isLikedBack ? (
                            <button
                              onClick={() => {
                                onAcceptLike(match);
                                setLikesSubTab('mutual');
                              }}
                              className="flex-1 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl py-2 px-4 text-sm hover:shadow-lg transition-all"
                            >
                              Принять приглашение
                            </button>
                          ) : (
                            <div className="flex-1 bg-emerald-50 text-emerald-700 rounded-xl py-2 px-4 text-sm text-center">
                              ✓ Взаимная симпатия!
                            </div>
                          )
                        ) : (
                          ...
                          <button
                            ...
                            className="flex-1 bg-gradient-to-r from-pink-500 to-rose-500 text-white rounded-xl py-2 px-4 text-sm hover:shadow-lg transition-all flex items-center justify-center gap-1"
                          >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 pb-6 text-white">
              ...
              <div className="flex gap-3">
                {!isLiked ? (
                  ...
                  <button
                    onClick={() => onRequestChat(currentMatch)}
                    className="flex-1 bg-white/95 backdrop-blur-md text-pink-600 rounded-2xl py-4 hover:bg-white transition-all flex items-center justify-center gap-2 shadow-xl"
                  >
                    <MessageCircle className="w-5 h-5" />
                    <span className="font-medium">Начать чат</span>
                  </button>
                ) : (
